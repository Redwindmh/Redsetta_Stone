...
Label(
    root, text="Hendricks Technomancy", font="arial 15 bold", bg="black", width="20"
).pack(side="bottom")
...
Output_text = Text(
    root, font="arial 12", height=11, wrap=WORD, padx=5, pady=5, width=60
)
Output_text.place(x=590, y=100)

t_engine = [
    "Google Translator",
    "ChatGpt Translator (3.5 Turbo)",
    "ChatGpt Translator (4o)",
    "DeepL Translator",
]
...
def Translate():
    translator = [
        ...
        ChatGptTranslator(api_key=OPENAI_API_KEY, model="gpt-4o-2024-08-06"),
        DeeplTranslator(api_key=DEEPL_API_KEY),
    ]

    translator[int(translation_service.current())].source = languages[
        int(translation_service.current())
    ][f"{src_lang.get()}"]
    translator[int(translation_service.current())].target = languages[
        int(translation_service.current())
    ][f"{targ_lang.get()}"]

    translated = (
        translator[int(translation_service.current())]
        .translate(text=Input_text.get(1.0, END))
        .strip(' " ')
    )
...
def open_file():
    file_path = askopenfile(
        mode="r", filetypes=[("Word files", "*.docx"), ("Excel files", "*.xlsx")]
    )
...
def upload_file():
    ...
    file = filedialog.asksaveasfile(
        initialdir="/Users/nephthys/Technomancy/Ouroboros/Rosetta_Stone/to_be_translated",
        filetypes=files,
        defaultextension=files,
    )
    pb1.destroy()
    Label(root, text="File uploaded successfully!", foreground="green").grid(
